...
HOST_DEVICE_INLINE int get_intersection_points(
  ...
  // vec中第一个点，是旋转框第二个角点的坐标减去第一个角点的坐标，即这两个角点的边所代表的向量
  // Line vector
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      ...
      // box1的某条边和box2的某条边的向量叉积
      // Solve for 2x2 Ax=b
      T det = cross_2d<T>(vec2[j], vec1[i]);

      // fabs取绝对值，叉积为0，表示两条边平行
      ...
      // 两条边的起点连成的向量，分别与这两边进行叉积
      T t1 = cross_2d<T>(vec2[j], vec12) / det;
      T t2 = cross_2d<T>(vec1[i], vec12) / det;

      // 边界框1的边，判断其两个点是否在交集内部
  ...
  // Check for vertices of rect1 inside rect2
  {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      // 如果P在AB的投影点在AB线上，P在AD的投影点在AD上，则P在ABCD内部
...
HOST_DEVICE_INLINE T rotated_boxes_intersection(
  ...
  // 最多返回24个交叉点
  // There are up to 4 x 4 + 4 + 4 = 24 intersections (including dups) returned from rotated_rect_intersection_pts
  ...
  // 凸包以顺时针顺序排列交点，并找到轮廓区域面积。
  // Convex Hull to order the intersection points in clockwise order and find the contour area.
...
single_box_iou_rotated(T const* const box1_raw, T const* const box2_raw) {
  ...
  // // 经过验证，发现有时候计算出来的intersection甚至比框的面积还大
  // // 对这种情况，我们直接返回0
  // if (intersection >= area1 || intersection >= area2){
  //   return 0.f;
  // }
